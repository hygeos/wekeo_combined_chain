...
# def _add_hover_dates(artists):
#     """Ajoute un tooltip affichant la date au survol des points/barres."""
#     cursor = mplcursors.cursor(artists, hover=True)

#     @cursor.connect("add")
#     def on_add(sel):
#         x_val = pd.Timestamp(matplotlib.dates.num2date(sel.target[0])).strftime("%Y-%m-%d")
#         sel.annotation.set_text(f"{x_val}\n{sel.target[1]:.3g}")
...
def plot_plume_timeseries(
    ...
    # SP -----------------------------------
    # line1, = ax.plot(times, plumes_count, marker='o', linestyle='-', linewidth=2, 
    #         label='Plumes (label < 100)', color='#2E86AB')
    # line2, = ax.plot(times, tiny_plumes_count, marker='s', linestyle='-', linewidth=2,
    #         label='Tiny Plumes (label ≥ 100)', color='#A23B72')
    
    # # Hover tooltip
    # _add_hover_dates([line1, line2])
    #----------------------------------------
...
#     ax.set_ylabel('Number of Detected Cells', fontsize=12, fontweight='bold')
...
#         title = f'Daily Detected Cells (s5p_pca__mean_score_CO) - ({np.datetime_as_string(start_date, unit="D")} to {np.datetime_as_string(end_date, unit="D")})'
...
def plot_detected_pixels_timeseries(
    ...
    """
    Plot timeseries of daily number of detected pixels as a bar chart.
    A pixel is considered detected if s5p_pca__mean_score_CO is not NaN and not a fill value.
    ...
    ax.bar(x, detected_pixels, width=0.6, color='#F18F01')
...
#     ax.set_ylabel('Number of FRP Cells', fontsize=12, fontweight='bold')
...
#         title = f'Daily FRP Cells (day/night MWIR) - ({np.datetime_as_string(start_date, unit="D")} to {np.datetime_as_string(end_date, unit="D")})'
...
def plot_frp_pixels_timeseries(
    ...
    """
    Plot timeseries of daily number of FRP pixels as a bar chart.
    A pixel is considered as FRP if either frp_slstr__day_FRP_MWIR_mean or 
    frp_slstr__night_FRP_MWIR_mean is not NaN and not a fill value.
    ...
    ax.bar(x, frp_pixels, width=0.6, color='#C73E1D')
...
# MAJ 18/06/2026
# ajout times series fire score & nb of sources detected
...
def plot_fire_score_timeseries(
    ...
    ax.bar(x - width / 2, df_summary["mean_fire_score"],
           width=width, color='gold', label='Daily mean fire score')
    ax.bar(x + width / 2, df_summary["max_fire_score"],
           width=width, color='b', alpha=0.7, label='Daily max fire score')
...
# ) -> Tuple[Figure, Axes]:
#     """
#     Plot timeseries of daily mean and max fire score.
...
#     ax.plot(df_summary.index, df_summary["mean_fire_score"],
#             marker='o', linestyle='-', linewidth=2, markersize=6,
#             color='gold', label='Daily mean fire score')
#     ax.plot(df_summary.index, df_summary["max_fire_score"],
#             marker='^', linestyle='--', linewidth=1.5, markersize=6,
#             color='b', label='Daily max fire score', alpha=0.7)
